...
- **12-factor config** — all secrets via environment variables; works locally, on Render, and on Railway
...
├── app.js              # Application logic (rendering + data layer)
...
If the numerator exceeds $R$, recovery is flagged as **mathematically impossible**.

### Trend classification
Compares the mean of the last 2 weekly entries against the prior weeks. Difference > 5 pp → IMPROVING; < −5 pp → DECLINING; otherwise STABLE.
